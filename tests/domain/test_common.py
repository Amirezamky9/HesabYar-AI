...
class TestMoneyIRR:
    ...
    def test_negative_without_adjustment_rejected(self) -> None:
        with pytest.raises(InvalidMoneyError, match="Negative monetary amount"):
            MoneyIRR(-500)

    def test_negative_with_adjustment_allowed(self) -> None:
        adj = MoneyIRR(-500, is_adjustment=True)
        assert adj.amount == -500
        assert adj.is_adjustment
        assert adj.is_negative

        adj2 = MoneyIRR.adjustment(-1200)
        assert adj2.amount == -1200
        assert adj2.is_adjustment
    ...
    def test_subtraction(self) -> None:
        ...
        # Underflow without adjustment authorization raises
        with pytest.raises(InvalidMoneyError, match="resulted in negative amount"):
            m2.sub(m1)

        # Underflow with allow_adjustment succeeds
        adj_res = m2.sub(m1, allow_adjustment=True)
        assert adj_res.amount == -20_000
        assert adj_res.is_adjustment
...
class TestFiscalDate:
    ...
    def test_from_jalali_constructor(self) -> None:
        fd = FiscalDate.from_jalali(1405, 1, 15)
    ...
    def test_moadian_epoch_ms_conversion(self) -> None:
        gd = datetime.date(2026, 4, 4)
        fd = FiscalDate.from_gregorian(gd)
        epoch_ms = fd.to_moadian_epoch_ms()
        assert isinstance(epoch_ms, int)

        # Convert back
        fd_back = FiscalDate.from_moadian_epoch_ms(epoch_ms)
        assert fd_back == fd
...
class TestContext:
    ...
    def test_admin_scope_bypasses_specific_checks(self) -> None:
        ...
        assert admin_actor.has_scope(Scope.LEDGER_POST)
        assert admin_actor.has_scope(Scope.TAX_SUBMIT)
        # Does not raise
    ...
    def test_actor_tenant_isolation_check(self) -> None:
        ...
        # Mismatch with tenant-b raises TenantMismatchError
    ...
    def test_system_admin_factory(self) -> None:
...
class TestPropertyInvariants:
    @given(
        a=st.integers(min_value=0, max_value=1_000_000_000_000),
        b=st.integers(min_value=0, max_value=1_000_000_000_000),
    )
    def test_money_addition_commutativity_and_associativity(self, a: int, b: int) -> None:
        m1 = MoneyIRR(a)
        m2 = MoneyIRR(b)
